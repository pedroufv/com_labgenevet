 ...
 * @since  1.6
 */
class LabgenevetModelContainers extends JModelList
{
    /**
	* Constructor.
	*
	...
	*/
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__labgenevet_containers` AS a');
    ...
    public function getAssocList($key = 'title', $value = 'id')
    {
        ...
        // Select the required fields from the table.
        $query->select('containers.'.$key.', containers.'.$value);
        $query->from('`#__labgenevet_containers` AS `containers`');
    ...
    {
        $containerslist = $this->getAssocList();

        if(count($containerslist) == 0) {
            return false;
        }

        $options = '';
        foreach ($containerslist as $key => $value) {
            $options .= '<option value="'.$value.'">'.$key.'</option>';
        }

        $xmlString = '<field name="containerslist" type="checkboxes" label="COM_LABGENEVET_CONTAINERS">'.$options.'</field>';
